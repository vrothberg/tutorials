...
run_podman_root run --annotation io.containers.trace-syscall=of:${tmp} $MINIMAGE ls /
clear
...
run_podman_root run --security-opt=seccomp=${tmp} $MINIMAGE ls / > /dev/null
clear

# And also that it won't work with `-l`.
run_podman_root run --security-opt=seccomp=${tmp} $MINIMAGE ls -l / > /dev/null
